...
var PORTRAIT_REGEX = RegEx.new()

func _init():
	PORTRAIT_REGEX.compile("(?<name>\\w+)(\\[(?<portrait>\\w+)\\])?")

func match_name(character_words):
	var r1 = PORTRAIT_REGEX.search(dialogue.character)
	if r1 == null:
		return character_words
	elif not r1.names.has("name"):
		return character_words
	else:
		return r1.strings[r1.names["name"]]

func show_potrait_pos(character_words):
	var r1 = PORTRAIT_REGEX.search(dialogue.character)
	if r1 == null:
		return DialogueManager.show_potrait(character_words)
	elif not r1.names.has("name"):
		return DialogueManager.show_potrait(character_words)
	else:
		if r1.names.has("portrait"):
			return DialogueManager.show_potrait(r1.strings[r1.names["name"]],r1.strings[r1.names["portrait"]])
		else:
			return DialogueManager.show_potrait(r1.strings[r1.names["name"]])
...
func _ready() -> void:
	...
	var name = match_name(dialogue.character)
	character_label.bbcode_text = "[wave]%s[/wave]" % name
...
#	dialogue_label.rect_size.y /= 4.0
	...
	var is_yield = DialogueManager.set_potrait_base(balloon.rect_global_position,balloon.rect_min_size)
	if is_yield is GDScriptFunctionState:
		is_yield = yield(is_yield,"completed")
	var await_result = show_potrait_pos(dialogue.character)
	if await_result is GDScriptFunctionState:
		await_result = yield(await_result,"completed")
	if await_result:
		character_label.bbcode_text = "[wave]%s[/wave]" % name
	else:
		character_label.bbcode_text = "[right][wave]%s[/wave][/right]" % name
