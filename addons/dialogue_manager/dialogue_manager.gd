...
func _ready() -> void:
	randomize()
	connect("dialogue_finished",self,"destroy_potrait")
...
func stop_conversation_right_now():
	cleanup()
	if is_instance_valid(current_ballon):
		current_ballon.queue_free()
	self.is_dialogue_running = false
...
func set_current_potrait_canvas(ins):
	get_tree().current_scene.add_child(ins)
	current_potrait_canvas = ins

func set_potrait_base(rect_gp,rect_s):
	if is_instance_valid(current_potrait_canvas):
		var result = current_potrait_canvas.set_potrait_base(rect_gp,rect_s)
		if result is GDScriptFunctionState:
			return yield(result,"completed")
	return true

func show_potrait(name,pos = null):
	if not is_instance_valid(current_potrait_canvas):
		return true
	return current_potrait_canvas.show_potrait(name,pos)

func destroy_potrait():
	if is_instance_valid(current_potrait_canvas):
		current_potrait_canvas.destroy_potrait()
